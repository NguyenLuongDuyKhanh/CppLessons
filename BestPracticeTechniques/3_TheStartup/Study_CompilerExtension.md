...
Great question — this is an important GNU C extension.

Let’s break down what `__attribute__((constructor))` really means.

---

### 🧩 Definition

`__attribute__((constructor))` is a **GCC and Clang extension** that marks a function to be **automatically executed before `main()`** runs — right after the C runtime is initialized.

It’s often used for:

* Initializing global state
* Registering plugins
* Running setup code before `main()` executes

---

### 🧠 How it works internally

When you compile a program, functions marked with `__attribute__((constructor))` are placed in a special section of the binary (like `.init_array` on ELF systems).
The **runtime loader** calls all those functions before calling `main()`.

It’s the same mechanism that runs global C++ constructors.
...
### 🧰 You can specify order too

If you have multiple constructors, you can assign a **priority** (lower number = runs earlier):

```c
__attribute__((constructor(101))) void first()  { printf("First\n"); }
__attribute__((constructor(102))) void second() { printf("Second\n"); }
```
...
| Property                                     | Meaning                                           |
| -------------------------------------------- | ------------------------------------------------- |
| 🏗️ Runs before `main()`                     | Even before global variables in `main()` are used |
| 🚫 No arguments                              | It must take no parameters and return `void`      |
| 🧩 Non-standard                              | Works on GCC, Clang, but not on pure MSVC         |
| 🧹 Paired with `__attribute__((destructor))` | Destructor runs after `main()` or on program exit |
...
| ------------------------------ | --------------- | ------------------- |
| `__attribute__((constructor))` | Before `main()` | Initialization code |
| `__attribute__((destructor))`  | After `main()`  | Cleanup code        |

---

Would you like me to show how this attribute looks in the compiled binary (like how GCC puts it in `.init_array`)? It’s a nice peek under the hood.
